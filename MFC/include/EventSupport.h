...
#define DECLARE_EVENT(access,f,p,v) \
	...
	{\
		...
		{\
			bool _b = false;\
			try{if(memcmp((BYTE*)_nPtrData,"################################################################################################################################",128)==0)_b=true;}catch(...){}\
...
#define DECLARE_EVENT_RECEIVER(classname) \
	...
	{\
		_event_Receiver(){memset(data,'#',128);nOffset=offsetof(classname,_xEventReceiver);}\
		~_event_Receiver(){memset(data,0,128);}\
		BYTE data[128];\
		int nOffset;\
	}_xEventReceiver;\
