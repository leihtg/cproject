...
BEGIN_MESSAGE_MAP(CTCPClientDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_BUTTON1, OnConn)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
void CTCPClientDlg::OnReceive(){
	BYTE buf[1024]={0};
	m_socket.Receive(buf,sizeof(buf),0);
	UpdateData();
	CString tmp;
	tmp.Format("接收到：%s\r\n",buf);
	m_szRecv+=tmp;
...
{
	UpdateData();
	m_socket.Send(m_szSend.GetBuffer(0),m_szSend.GetLength(),0);
}
void CTCPClientDlg::OnCls(){
	AfxMessageBox("socket已关闭");
}

void CTCPClientDlg::OnConn() 
{
	m_socket.Connect("127.0.0.1",10000);
}
